...
local defaults = {
	global = {
		settings = {
			...
			--[[
			runspeed = 100% = ~7yards per second, max clocked speed 550% (Flightpath)
...
update:SetScript("OnUpdate", function(self, elapsed)
	...
	zone_w, zone_h = GatherMate:GetZoneSize(zoneID)
...
function GatherHud:GetOrCreatePin( unique_id, coord, texture_string )
	...
	pin[X], pin[Y] = GatherMate.mapData:DecodeLoc( coord )
...
function GatherHud:FullUpdate(force)
	...
	for i,db_type in pairs(GatherMate.db_types) do
		if GatherMate.Visible[db_type] then
			...
			for coord, nodeID in GatherMate:FindNearbyNode(zoneID, cx, cy, level, db_type, settings.radius+settings.look_ahead) do
